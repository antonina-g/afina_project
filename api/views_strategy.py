...
def generate_strategy_view(request, user_id):
    """
    Явная генерация стратегии обучения для пользователя и курса.
...
def get_user_strategies(request, user_id):
    ...
    try:
        ...
        return Response({"strategies": data}, status=status.HTTP_200_OK)
...
def get_course_strategy(request, user_id, course_id):
    ...
    Если стратегии нет — создаёт её через LLM/заглушку.
    Можно расширить логикой refresh (?refresh=1) при необходимости.
    """
    try:
        if request.user.id != int(user_id):
            return Response(
        ...
        try:
            ...
            # Получаем свежесозданную стратегию из БД
            strategy = LearningStrategy.objects.get(user=profile, course=course)
